from pathlib import Path

BUCKET_NAME = "systematic-trading-infra-storage"
S3_OBJECT_NAMES = [
    "backtest/wrds_universe_prices.parquet",
    "data/ib_historical_prices.parquet"
]

MAX_CONSECUTIVE_NAN = 5
REBASE_PRICES = False
N_IMPLEMENTATION_LAGS = 1
FORMAT_DATE = "%Y-%m-%d"
NB_PERIOD_MOM = 22*12
NB_PERIOD_TO_EXCLUDE_MOM = 22*1
EXCLUDE_LAST_PERIOD_MOM = True
PCT_WINSO = (2,98)
PCT_PTF = (10,90)
INDUSTRY_SEG = None
REBAL_PERIODS = 22
PTF_TYPE = "long_only"
REBAL_PERIODS_BENCH = 22
PTF_TYPE_BENCH = "long_only"
TC = 10
TC_BENCH = 10
STRAT_NAME = "LO CSMOM"
STRAT_NAME_BENCH = "Bench Buy-and-Hold EW"
FREQ_DATA = "d"

ROOT_PATH = Path(__file__).resolve().parents[1]
SAVING_PATHS = [
    ROOT_PATH / "outputs" / "figures" / "cum_perf_backtest_wrds.png",
    ROOT_PATH / "outputs" / "figures" / "cum_perf_backtest_ib.png"
]
